...
PROFILES = {
    'Balanced Value': {
        ...
        'min_recent_played': 1,
        'kpi_weights': {
            'GK':{'recent_avg':0.05,'season_avg':0.70,'regularity':0.25,'recent_goals':0,'season_goals':0},
            'DEF':{'recent_avg':0.25,'season_avg':0.25,'regularity':0.25,'recent_goals':0,'season_goals':0},
    ...
    'Attack Focus': {
        ...
        'kpi_weights': { 'GK':{'recent_avg':0,'season_avg':0.5,'regularity':0.5,'recent_goals':0,'season_goals':0},
                         'DEF':{'recent_avg':0,'season_avg':0.6,'regularity':0.4,'recent_goals':0,'season_goals':0},
                         'MID':{'recent_avg':0.1,'season_avg':0.2,'regularity':0.1,'recent_goals':0.3,'season_goals':0.3},
                         'FWD':{'recent_avg':0.1,'season_avg':0.2,'regularity':0.1,'recent_goals':0.3,'season_goals':0.3} },
        'mrb_params': { 'GK':0.2,'DEF':0.3,'MID':0.7,'FWD':1.2 }
    },
    'Defensive Focus': {
        'n_recent_games': 5,'min_recent_played':1,
        'kpi_weights': { 'GK':{'recent_avg':0.1,'season_avg':0.6,'regularity':0.3,'recent_goals':0,'season_goals':0},
                         'DEF':{'recent_avg':0.3,'season_avg':0.3,'regularity':0.2,'recent_goals':0,'season_goals':0},
                         'MID':{'recent_avg':0.1,'season_avg':0.2,'regularity':0.2,'recent_goals':0.2,'season_goals':0.2},
                         'FWD':{'recent_avg':0,'season_avg':0.2,'regularity':0.1,'recent_goals':0.3,'season_goals':0.3} },
...
class MPG:
    ...
    def simplify_position(pos:str)->str:
        ...
        p=pos.strip().upper()
        return ('GK' if p=='G' else 'DEF' if p in ['D','DC','DL','DF'] else
                'MID' if p in ['M','MD','MO','MC'] else
                'FWD' if p in ['A','AT','FW'] else 'UNKNOWN')
...
# --- App ---
def main():
    st.markdown('<div class="header">🚀 MPG Auction Helper</div>', unsafe_allow_html=True)
    uploaded=st.sidebar.file_uploader("Upload season data (CSV/XLSX)", type=['csv','xls','xlsx'])
    prof=st.sidebar.selectbox("Profile", list(PROFILES.keys()))
    if prof!='Custom': params=PROFILES[prof]
    else:
        st.sidebar.markdown("Select Custom parameters... (not shown)")
        return
    formation=st.sidebar.selectbox("Formation", ['4-4-2','4-3-3','3-5-2','3-4-3','4-5-1','5-3-2','5-4-1'])
    minima={'GK':2,'DEF':6,'MID':6,'FWD':4}
    squad_size=st.sidebar.number_input("Squad Size", min_value=sum(minima.values()), value=20)
    budget=st.sidebar.number_input("Budget", value=500)
