...
def get_classement(championnat_scores, adversaires):
    if not isinstance(adversaires, list):
        adversaires = [adversaires] if adversaires else []
...
with tab_coupe:
    ...
    with subtab2:
        ...
        with col_prev:
            if idx > 0 and st.button("←", key="tour_prev"):
        ...
        with col_next:
            if idx < len(tours) - 1 and st.button("→", key="tour_next"):
                st.session_state.selected_tour = tours[idx + 1]
                st.rerun()
        ...
        for i, match in enumerate(matchs):
            col1, col2, col3, col4, col5 = st.columns([3, 1, 1, 3, 1])
            dom = col1.selectbox(f"Domicile {i+1}", equipes, index=equipes.index(match["domicile"]), key=f"coupe_dom_{selected}_{i}")
            score_dom = col2.number_input("⚽", value=match.get("score_dom", 0), min_value=0, max_value=30, key=f"coupe_score_dom_{selected}_{i}")
            score_ext = col3.number_input("⚽", value=match.get("score_ext", 0), min_value=0, max_value=30, key=f"coupe_score_ext_{selected}_{i}")
            ext = col4.selectbox(f"Extérieur {i+1}", equipes, index=equipes.index(match["exterieur"]), key=f"coupe_ext_{selected}_{i}")
            matchs[i] = {
                "domicile": dom,
                "score_dom": score_dom,
                "exterieur": ext,
        ...
        st.markdown("---")
        # Ajouter une nouvelle journée
        if st.button("🗓️ Ajouter un tour"):
    ...
    with subtab3:
        ...
        if st.button("💾",key=f"save_coupe_adv_{adv_df}"):
