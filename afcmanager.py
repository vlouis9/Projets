...
# Import Data
uploaded_file = st.sidebar.file_uploader("Import JSON Data", type=['json'], key="json_uploader")
...
with tab1:
    ...
    # Editable player table with unique key
    ...
    if st.button("Sauvegarder les modifications", key="save_players_btn"):
...
with tab2:
    ...
    with tab2_1:
        ...
        # Lineup name with unique key
        lineup_name = st.text_input("Nom de la composition", key="lineup_name_create")
        
        # Formation selector with unique key
        formation = st.selectbox(
            "Formation",
            list(FORMATIONS.keys()),
            key="formation_selector_create"
        )
            ...
            with cols[idx % 3]:
                ...
                if selected_player:
                    ...
                    lineup_dict["numbers"][selected_player] = st.number_input(
                        ...
                        max_value=99,
                        value=idx + 1,
                        key=f"lineup_num_{idx}"
                    )
        
        # Substitutes with unique keys
            ...
            with sub_cols[i]:
                ...
                if sub:
                    ...
                    lineup_dict["numbers"][sub] = st.number_input(
                        ...
                        max_value=99,
                        value=12+i,
                        key=f"lineup_sub_num_{i}"
                    )
        
        # Captain selection with unique key
        captain_options = [p for p in lineup_dict["players"].values() if p]
        if captain_options:
            lineup_dict["captain"] = st.selectbox(
                "Captain",
        ...
        # Save lineup with unique key
        if st.button("Sauvegarder la composition", key="save_lineup_btn"):
            if not lineup_name:
...
with tab2_2:
    ...
    for idx, lineup in enumerate(st.session_state.data["lineups"]):
        with st.expander(f"{lineup['name']} ({lineup['formation']})"):
            ...
            # Edit and Delete buttons with unique keys
            col1, col2 = st.columns(2)
            with col1:
                if st.button("Edit", key=f"edit_lineup_{idx}"):
                    st.session_state.lineup_to_edit = idx
            with col2:
                if st.button("Delete", key=f"delete_lineup_{idx}"):
...
with tab3:
    ...
    with tab3_1:
        ...
        # Match details form with unique keys
        col1, col2 = st.columns(2)
        with col1:
            match_type = st.selectbox("Type", ["Championship", "Cup"], key="match_type_create")
            opponent = st.text_input("Opponent", key="match_opponent_create")
            match_date = st.date_input("Date", key="match_date_create")
        with col2:
            match_time = st.time_input("Time", key="match_time_create")
            location = st.text_input("Location", key="match_location_create")
        ...
        selected_lineup = st.selectbox("Select lineup", [""] + saved_lineups, key="match_lineup_select")
...
if selected_lineup:
    ...
    formation = st.selectbox("Formation", list(FORMATIONS.keys()), key="match_formation_create")
    initial_players = {}
    initial_subs = []
...
# Player selection interface with unique keys
st.subheader("Starting Lineup")
positions = FORMATIONS[formation]["positions"]
coordinates = FORMATIONS[formation]["coordinates"]
...
if match_dict["players"]:
    ...
    # Save match button with unique key
    if st.button("Sauvegarder le match", key="save_match_btn"):
        if not opponent:
    ...
    for idx, match in enumerate(st.session_state.data["matches"]):
        with st.expander(f"{match['name']} ({match['type']})"):
            if not match["completed"]:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    afc_score = st.number_input(
                        "AFC Score",
                        min_value=0,
                        value=match["score"]["AFC"],
                        key=f"afc_score_{idx}"
                    )
                with col2:
                    opp_score = st.number_input(
                        f"{match['opponent']} Score",
                        min_value=0,
                        value=match["score"]["opponent"],
                        key=f"opp_score_{idx}"
                    )
                ...
                for i in range(max(afc_score, len(match["goals"]))):
                    ...
                    with col2:
                        assist = st.selectbox(
                            ...
                            index=all_players.index(match["goals"][i]["assist"])+1 if i < len(match["goals"]) and match["goals"][i]["assist"] else 0
                        )
                    if scorer:
